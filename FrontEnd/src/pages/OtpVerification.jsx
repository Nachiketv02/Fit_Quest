...
function OtpVerification() {
  ...
  const handleVerify = async () => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/fit-quest/users/verify`,
        { email: userData, verificationCode: Number(otp.join("")) },
        {
  ...
  const handleResendOtp = async () => {
    ...
    try {
      await axios.post(
        `${import.meta.env.VITE_BASE_URL}/fit-quest/users/resend`,
        { email: userData }
      );
        ...
        {isVerified ? (
          ...
          <>
            <div className="flex items-center">
              ...
              <h2 className="text-center text-2xl font-bold text-gray-900 flex-1 pr-8">
                Verify Your Account
              </h2>
            </div>
            <div className="mt-6 text-center">
              ...
              {error && (
                <motion.p className="mt-4 text-red-500">{error}</motion.p>
              )}
              <motion.button
                onClick={handleVerify}
                disabled={isVerifying || otp.includes("")}
                className="mt-8 w-full py-3 text-white bg-primary hover:bg-primary/90"
              >
